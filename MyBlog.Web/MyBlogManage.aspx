<%@ Page Language="C#" AutoEventWireup="true" CodeFile="MyBlogManage.aspx.cs" Inherits="MyBlogManage" %>
    ...
    <script>
        layui.use(['table', 'jquery'], function () {
            ...
            table.render({
                elem: '#blogTable'
                , height: 444//高度
                , id: 'blogTable'//表格id，搜索功能重载表格需要使用
                , url: '/Comm/ApiGetBlog.ashx'
                ...
                , cols: [[
                    ...
                    , { field: 'blogId', title: '博客ID', width: 120, fixed: 'left', unresize: true, sort: true }
                    , { field: 'typeId', title: '博客类别ID', width: 120, sort: true }
                    , {
                        field: 'blogTitle', title: '博客标题', width: 200, templet: function (res) {
                            return '<em>' + res.blogTitle + '</em>'
                        }
                    }
                    , { field: 'blogContent', title: '博客内容', width: 180 }
                    , { field: 'blogTime', title: '发布时间', width: 180, sort: true }
            ...
            table.on('tool(test)', function (obj) {
                ...
                //console.log(obj)
                if (obj.event === 'del') {
                    layer.confirm('真的删除博客:' + data.blogTitle + '?', function (index) {
                        $.ajax({
                            type: "post",
                            async: true,
                            url: "/Comm/ApiDeleteBlog.ashx",
                    ...
                    layer.prompt({
                        ...
                        $.ajax({
                            type: "post",
                            async: true,
                            url: "/Comm/ApiEditBlog.ashx",
                            data: { blogId: data.blogId, blogTitle: data.blogTitle, blogContent: value },
                            success: function (res) {
                                console.log(res);
                            }
                        })
            ...
            var $ = layui.$, active = {
                reload: function () {
                    ...
                    table.reload('blogTable', {
                        url: '/Comm/ApiFuzzyBlog.ashx'//模糊查询接口，默认是table.render中的url
                        ...
                        , where: {//设定异步数据接口的额外参数，任意设
                            blogTitleOrContent: demoReload.val()
                        }
                    }, 'data');
                }
            };
